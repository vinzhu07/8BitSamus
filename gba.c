...
void drawImageDMA(int row, int col, int width, int height, const u16 *image)
{
  for (int i = 0; i < height; i++)
  {
    ...
    DMA[3].cnt = DMA_ON | width | DMA_DESTINATION_INCREMENT | DMA_SOURCE_INCREMENT | DMA_CHANNEL_3;
  } // TODO: IMPLEMENT
  // UNUSED(row);
  // UNUSED(col);
  // UNUSED(width);
...
void fillScreenDMA(volatile u16 color)
{
  DMA[3].src = &color;
  DMA[3].dst = videoBuffer;
  DMA[3].cnt = WIDTH * HEIGHT | DMA_ON | DMA_16 | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT | DMA_CHANNEL_3;
